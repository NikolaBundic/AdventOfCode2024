...
public class Second {
    public static void main(String[] args) throws IOException {
        ...
        //Just guessed that it would happen when its safest because they are all close to each other, and it makes sense with the first part.
        int lowest = 0;
        long safety = Long.MAX_VALUE;
        for(int p = 1; p < 10000; p++) {
            int[][] matrix = new int[matY][matX];

            for (Robot robot : robots) {
                int x = robot.vx * p % matX + robot.px;
                int y = robot.vy * p % matY + robot.py;
                if (x >= matX) {
                    x -= matX;
                } else if (x < 0) {
                    x += matX;
            ...
            if((long) count1 * count2 * count3 * count4 < safety){
                System.out.println(p);
                safety = (long) count1 * count2 * count3 * count4;
                lowest = p;
            }
        }
