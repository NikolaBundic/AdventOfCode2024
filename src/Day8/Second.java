        ...
        for (Map.Entry<Character, ArrayList<ArrayList<Integer>>> entry : map.entrySet()) {
            ...
            for (int i = 0; i < positions.size(); i++) {
                ...
                for (int j = i + 1; j < positions.size(); j++) {
                    ...
                    while (pos1.get(0) - posY * mult >= 0) {
                        if (pos1.get(1) > pos2.get(1)) {
                            if (pos1.get(1) + posX * mult >= matrix.getFirst().size()) {
                                break;
                            }
                            matrix.get(pos1.get(0) - posY * mult).set(pos1.get(1) + posX * mult, '#');
                        } else if (pos1.get(1) < pos2.get(1)) {
                            if (pos1.get(1) - posX * mult < 0) {
                                break;
                            }
                            matrix.get(pos1.get(0) - posY * mult).set(pos1.get(1) - posX * mult, '#');
                        } else {
                            matrix.get(pos1.get(0) - posY * mult).set(pos1.get(1), '#');
                        }
                    ...
                    while (pos2.get(0) + posY * mult < matrix.size()) {
                        if (pos1.get(1) > pos2.get(1)) {
                            if (pos2.get(1) - posX * mult < 0) {
                                break;
                            }
                            matrix.get(pos2.get(0) + posY * mult).set(pos2.get(1) - posX * mult, '#');
                        } else if (pos1.get(1) < pos2.get(1)) {
                            if (pos2.get(1) + posX * mult >= matrix.getFirst().size()) {
                                break;
                            }
                            matrix.get(pos2.get(0) + posY * mult).set(pos2.get(1) + posX * mult, '#');
                        } else {
                            matrix.get(pos2.get(0) + posY * mult).set(pos2.get(1), '#');
                        }
